...
Video::Video(Resource *res)
	...
	_background_bitmap_ptr = nullptr;
}
...
void Video::setFont(const uint8_t *font) {
	debug(DBG_INFO, "setFont %p", font);
	int w, h;
	uint8_t *buf = decode_bitmap(font, true, -1, &w, &h, BITMAP_TYPE_FONT);
	if (buf) {
	...
	{
		_graphics->setFont(nullptr, 128, 128);
	}
}

void Video::setHeads(const uint8_t *src, bool flipY) {
	debug(DBG_INFO, "setHeads");
	int w, h;
	uint16_t *buf = decode_bitmap_toRGB5551(src, true, 0xF06080, &w, &h, flipY);
	// uint8_t *buf = decode_bitmap(src, true, 0xF06080, &w, &h, BITMAP_TYPE_HEADS, flipY);
	if (buf) {
		_graphics->setSpriteAtlas((uint8_t *)buf, w, h, 2, 2);
		free(buf);
...
void Video::drawString(uint8_t color, uint16_t x, uint16_t y, uint16_t strId) {
	...
	debug(DBG_INFO, "drawString(%d, %d, %d, '%s', %d)", color, x, y, str, strId);
	uint16_t xx = x;
	int len = strlen(str);
	for (int i = 0; i < len; ++i) {
		if (str[i] == '\n' || str[i] == '\r') {
...
void Video::scaleBitmap(const uint8_t *src, int fmt) {
	if (_scaler) {
		...
		_graphics->drawBitmap(_res->getDataType() == Resource::DT_3DO ? 0 : _buffers[0], _scalerBuffer, w, h, fmt);
	} else {
		_graphics->drawBitmap(_res->getDataType() == Resource::DT_3DO ? 0 : _buffers[0], src, BITMAP_W, BITMAP_H, fmt);
	}
}
...
void Video::copyBitmapPtr(const uint8_t *src, uint32_t size) {
	if (_res->getDataType() == Resource::DT_DOS || _res->getDataType() == Resource::DT_AMIGA) {
		...
		debug(DBG_INFO, "copyBitmapPtr %ld", size);
		...
		if (Graphics::_is1991) {
			...
			int w, h;
			uint8_t *buf = decode_bitmap(src, false, -1, &w, &h, BITMAP_TYPE_BACKGROUND);
			if (buf) {
				_graphics->drawBitmap(_buffers[0], buf, w, h, FMT_RGB);
				// free(buf);  _background_bitmap_ptr = nullptr; // Don't free background bitmaps
			}
		}
	}
}
